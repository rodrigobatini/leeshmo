    ...
    <div className="space-y-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-1 max-w-xl text-sm text-muted-foreground">
            Perfis do onboarding — tom, dor e CTA por canal.
          </p>
        </div>
        {personas.length > 0 ? (
      ...
      {summary ? (
        <p className="rounded-lg border border-border/50 bg-card/60 px-3 py-2 text-xs text-muted-foreground sm:text-sm">
          <span className="font-medium text-foreground">Contexto:</span> {summary}
        </p>
      ) : null}
      ...
      {personas.length === 0 ? (
        <Card className="overflow-hidden border-dashed">
          <CardContent className="flex flex-col items-center px-5 py-10 text-center sm:py-12">
            ...
            <h2 className="text-base font-semibold">Nenhuma persona salva ainda</h2>
              ...
              <Link href={`/personas/${encodeURIComponent(p.name)}`} className="block h-full">
                ...
                >
                  ...
                  <CardContent className="space-y-1.5 p-3 pt-0 text-xs text-muted-foreground sm:p-4 sm:pt-0">
                    <p>
                      <span className="font-medium text-foreground">Objetivo:</span> {p.objective}
                    </p>
                    <p className="line-clamp-2">
                      <span className="font-medium text-foreground">Tom:</span> {p.tone}
                    </p>
                  </CardContent>
                </Card>
              </Link>
