...
export function AiUsageHint({ className }: { className?: string }) {
  ...
  return (
    <div
      ...
      title={empty ? "Limite diario atingido — renova em UTC meia-noite" : `Restam ${q.remaining} geracoes hoje`}
    >
      <span className="flex items-center gap-1 font-medium text-foreground">
        <Sparkles className="h-3 w-3 shrink-0 text-primary" />
        <span className="hidden sm:inline">Uso de IA hoje</span>
      </span>
      <span className="tabular-nums text-muted-foreground">
        {empty ? "Renova em breve" : (
          <>
            <span className="sm:hidden">{q.remaining}/{q.limit}</span>
            <span className="hidden sm:inline">{`${q.remaining} de ${q.limit} restantes`}</span>
          </>
        )}
      </span>
    </div>
